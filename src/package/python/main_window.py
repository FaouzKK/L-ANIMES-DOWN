...
class MainWindow(QtWidgets.QWidget):
    ...
    def modify_widgets(self):
        ...
        self.welcome_message.setAlignment(QtCore.Qt.AlignmentFlag.AlignTop | QtCore.Qt.AlignmentFlag.AlignLeft)
        self.welcome_message.setStyleSheet("font-size: 18px; font-weight: bold ; background-color: rgba(240,240,240,0.5);")
        self.welcome_message.setSizePolicy(QtWidgets.QSizePolicy.Policy.Fixed, QtWidgets.QSizePolicy.Policy.Fixed)
...
""")
    ...
    def load_anime_list(self, anime_list):
        items_list = [self.anime_list.item(i).text() for i in range(self.anime_list.count())]
    ...
    def printError(self, error = 'hi'):
        QtWidgets.QMessageBox.information(self, "Erreur",error, QtWidgets.QMessageBox.StandardButton.Close)
        self.enable_all()
        self.anime_laoding_button.stop_loading()
    ...
    def show_chapiters_in_right_widget(self,item=None,*,force_reload=False):
        ...
        animeLink = next(content['link'] for content in anime if content['name'] == content_name)
        ...
        self.chapiters_widget = MWidget.ChapitersWidget(anime_name,content_name,language)
        self.chapiters_widget.loading_button.clicked.connect(partial(self.show_chapiters_in_right_widget, force_reload=True))
